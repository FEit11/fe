...
<script>
    ...
    layui.extend({
    ...
    }).use(['dtree','form', 'table', 'laydate', 'jquery','layer'], function () {
        var $ = layui.jquery,
            form = layui.form,
        ...
        function deleteById(data) {
            //判断当前角色下是否存在员工
            $.get("/admin/role/checkEmployeeByRoleId",{"id":data.id},function(result){
                if(result.exist){
                    //提示用户无法删除
                    layer.msg(result.message);
                }else{
                    //提示用户是否删除该角色
                    layer.confirm("确定删除["+data.roleName+"]?", {icon: 3, title:'提示'}, function(index){
                        //发送ajax请求进行删除
                        $.post("/admin/role/deleteByRoleId",{"id":data.id},function(result){
                            if(result.success){
                                //刷新数据表格
                                tableIns.reload();
                            }
            ...
            mainIndex = layer.open({
                ...
                success: function () {
                    ...
                    dtree.render({
                        ...
                        checkbar: true,//true表示开启复选框，false关闭
